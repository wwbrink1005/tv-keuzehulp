...
function buildSpecList(printer) {
  const specs = [];

  if (printer.printtechnologie) {
    specs.push(printer.printtechnologie);
  }

  if (printer.gebruikType) {
    const labels = { thuis: "Thuisgebruik", zakelijk: "Zakelijk gebruik", foto: "Fotoprinter" };
    specs.push(labels[printer.gebruikType] ?? printer.gebruikType);
  }

  if (printer.printsnelheidZwart) {
    specs.push(`${printer.printsnelheidZwart} ppm`);
  }

  if (printer.kanKleurenPrinten === "Ja") {
    specs.push("Kleur");
  }
...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "Printer die bij je past"
      : "Printers die bij je passen";
  }
}
...
function displayOtherMatchesRedesign(filteredMatchedPrinters) {
  ...
  if (filteredMatchedPrinters.length === 0) {
    container.innerHTML = '<p class="no-matches">Geen passende printers gevonden.</p>';
    return;
  }

  const prices = filteredMatchedPrinters.map(p => parsePrice(p.prijs));
  const minPrice = Math.min(...prices);

  container.innerHTML = filteredMatchedPrinters
    .map((printer, index) => {
      const price = parsePrice(printer.prijs);
      const isCheapest = price === minPrice;
      const specs = buildSpecList(printer);
      const points = buildResultPoints(printer, currentAnswers);

      return buildCardHtml({ product: printer, index, isCheapest, specs, points });
    })
    .join("");
...
export function initResultPage() {
  ...
  const bestMatchData = localStorage.getItem("printer_bestMatch");
  const filteredData  = localStorage.getItem("printer_filteredMatchedPrinters");
  const answersData   = localStorage.getItem("printer_answers");

  if (!bestMatchData) return;

  const rawFilteredMatchedPrinters = filteredData ? JSON.parse(filteredData) : [];
  const answers                    = answersData  ? JSON.parse(answersData)  : null;

  const { result: filteredMatchedPrinters } = applyMinAanbiedersCascade(
    Array.isArray(rawFilteredMatchedPrinters) ? rawFilteredMatchedPrinters : []
  );

  currentAnswers = answers;
  baseMatches    = filteredMatchedPrinters;
