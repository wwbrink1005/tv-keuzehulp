...
/**
 * Adapts a single vriezers row to the shape that normalizeProducts() expects.
 */
function adaptRow(row) {
  return {
    ...
    klimaatklasse:          row.klimaatklasse ?? "",
...
/**
 * Fetches all rows from the `vriezers` table with offset-based pagination.
 */
async function fetchAll() {
  ...
  while (true) {
    const url = `${SUPABASE_URL}/rest/v1/vriezers?order=ean&limit=${PAGE_SIZE}&offset=${offset}`;
    const response = await fetch(url, { headers: HEADERS });

    if (!response.ok) {
      throw new Error(
        `Supabase fetch mislukt (vriezers): ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Fetches all vriezers and returns them in the shape normalizeProducts() expects.
 */
