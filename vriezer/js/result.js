...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "De vriezer die het beste bij je past!"
      : "De vriezers die het beste bij je passen!";
  }
}
...
function displayOtherMatchesRedesign(filteredMatchedVriezers) {
  ...
  container.innerHTML = filteredMatchedVriezers
    .map((vriezer, index) => {
      ...
      `).join("");
      const providersHtml = buildProvidersHtml(vriezer.aanbieders);

      return `
        <article class="tv-card${isCheapest ? " is-cheapest" : ""}" data-match-index="${index}">
          <div class="tv-card-image" aria-hidden="true">
            <img src="${vriezer.afbeelding || ''}" alt="" role="presentation" ${index < 4 ? 'fetchpriority="high" loading="eager"' : 'loading="lazy"'} onerror="this.onerror=null;this.src=window.IMG_FALLBACK;">
            <button class="tv-preview-btn" type="button" aria-label="Afbeelding vergroten" data-preview-src="${vriezer.afbeelding || ''}" data-preview-name="${vriezer.naam}" data-preview-imgs="${JSON.stringify(vriezer.afbeeldingen || []).replace(/"/g, '&quot;')}">
              <i data-lucide="eye"></i>
            </button>
          </div>
          <div class="tv-card-body">
            ${isCheapest ? '<span class="tv-card-cheapest-badge">Goedkoopste keuze</span>' : ''}
            <h3 class="tv-card-name">${vriezer.naam}</h3>
...
export function initResultPage() {
  ...
  const filteredMatchedVriezers = filteredData ? JSON.parse(filteredData) : [];
  const answers                 = answersData  ? JSON.parse(answersData)  : null;

  currentAnswers = answers;
  baseMatches    = Array.isArray(filteredMatchedVriezers) ? filteredMatchedVriezers : [];
