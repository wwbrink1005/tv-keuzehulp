...
function buildSpecList(soundbar) {
  const specs = [];

  if (soundbar.kanalen) {
    // Icecat's waarde bevat het woord "kanalen" al (bijv. "3.1.2 kanalen").
    specs.push(soundbar.kanalen);
  }

  if (soundbar.subwoofer_meegeleverd === "Ja") {
    specs.push("Incl. subwoofer");
  }

  if (soundbar.vermogen_watt) {
    specs.push(`${soundbar.vermogen_watt}W`);
  }

  if (soundbar.breedte_mm) {
    specs.push(`${Math.round(soundbar.breedte_mm / 10)} cm breed`);
  }
...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "De soundbar die het beste bij je past!"
      : "De soundbars die het beste bij je passen!";
  }
}

function displayOtherMatchesRedesign(filteredMatchedSoundbars) {
  resetProvidersRegistry();
  const container = qs("#otherMatchesGrid");
  if (!container) return;

  if (filteredMatchedSoundbars.length === 0) {
    container.innerHTML = '<p class="no-matches">Geen passende soundbars gevonden.</p>';
    return;
  }

  const prices = filteredMatchedSoundbars.map(sb => parsePrice(sb.prijs));
  const minPrice = Math.min(...prices);

  container.innerHTML = filteredMatchedSoundbars
    .map((soundbar, index) => {
      const price = parsePrice(soundbar.prijs);
      const isCheapest = price === minPrice;
      const specs = buildSpecList(soundbar);
      const specsText = specs.join(" • ");
      const points = buildResultPoints(soundbar, currentAnswers);
      ...
      `).join("");
      const providersHtml = buildProvidersHtml(soundbar.aanbieders);

      return `
        <article class="tv-card${isCheapest ? " is-cheapest" : ""}" data-match-index="${index}">
          <div class="tv-card-image" aria-hidden="true">
            <img src="${soundbar.afbeelding || ''}" alt="" role="presentation" ${index < 4 ? 'fetchpriority="high" loading="eager"' : 'loading="lazy"'} onerror="this.onerror=null;this.src=window.IMG_FALLBACK;">
            <button class="tv-preview-btn" type="button" aria-label="Afbeelding vergroten" data-preview-src="${soundbar.afbeelding || ''}" data-preview-name="${soundbar.naam}" data-preview-imgs="${JSON.stringify(soundbar.afbeeldingen || []).replace(/"/g, '&quot;')}">
              <i data-lucide="eye"></i>
            </button>
          </div>
          <div class="tv-card-body">
            ${isCheapest ? '<span class="tv-card-cheapest-badge">Goedkoopste keuze</span>' : ''}
            <h3 class="tv-card-name">${soundbar.naam}</h3>
...
export function initResultPage() {
  ...
  const bestMatchData  = localStorage.getItem("soundbar_bestMatch");
  const bestType       = localStorage.getItem("soundbar_bestType");
  const filteredData   = localStorage.getItem("soundbar_filteredMatchedSoundbars");
  const answersData    = localStorage.getItem("soundbar_answers");

  if (!bestMatchData) return;

  const rawFilteredMatchedSoundbars = filteredData ? JSON.parse(filteredData) : [];
  const answers                     = answersData  ? JSON.parse(answersData)  : null;

  const { result: filteredMatchedSoundbars } = applyMinAanbiedersCascade(
    Array.isArray(rawFilteredMatchedSoundbars) ? rawFilteredMatchedSoundbars : []
  );

  currentAnswers = answers;
  baseMatches    = filteredMatchedSoundbars;
