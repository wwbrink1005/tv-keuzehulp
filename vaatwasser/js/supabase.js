...
/**
 * Extracts the numeric couvert count from strings like "14 couverts" → 14.
 */
function parseCouverts(value) {
...
/**
 * Extracts the first integer from strings like "210 min" → 210, "42 dB" → 42.
 */
...
/**
 * Adapts a single vaatwassers row to the shape that normalizeProducts() expects.
 */
function adaptRow(row) {
  return {
    ...
    titel:                 row.titel ?? "",
    merk:                  row.merk,
    apparaatplaatsing:     row.plaatsing ?? "",
    couverts:              parseCouverts(row.couverts),
    geluidDb:              parseFirstInt(row.geluid_db),
    energieLabel:          String(row.energie_label ?? "").trim().toUpperCase(),
    cyclusDuurMin:         parseFirstInt(row.cyclus_duur_min),
    afwasprogrammas:       row.afwasprogrammas ?? "",
    droogsysteem:          row.droogsysteem ?? "",
    droogprestaties:       String(row.droogprestaties ?? "").trim().toUpperCase(),
    bestekType:            row.bestek_type ?? "",
    verstelbareBovenkorf:  parseJaNee(row.verstelbare_bovenkorf),
    halveLading:           parseJaNee(row.halve_lading),
    ...
    glasbescherming:       parseJaNee(row.glasbescherming),
    automatischeDeuropening: parseJaNee(row.automatische_deuropening),
    vloerlampje:           parseJaNee(row.vloerlampje),
    kleur:                 row.kleur ?? "",
    breedteMm:             parseFirstInt(row.breedte),
    diepteMm:              parseFirstInt(row.diepte),
    hoogteMm:              parseFirstInt(row.hoogte),
...
/**
 * Fetches all rows from the `vaatwassers` table with offset-based pagination.
 */
async function fetchAll() {
  ...
  while (true) {
    const url = `${SUPABASE_URL}/rest/v1/vaatwassers?order=ean&limit=${PAGE_SIZE}&offset=${offset}`;
    const response = await fetch(url, { headers: HEADERS });

    if (!response.ok) {
      throw new Error(
        `Supabase fetch mislukt (vaatwassers): ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Fetches all vaatwassers and returns them in the shape normalizeProducts() expects.
 */
