...
function buildSpecList(vaatwasser) {
  const specs = [];

  if (vaatwasser.couverts) {
    specs.push(`${vaatwasser.couverts} couverts`);
  }

  if (vaatwasser.plaatsing) {
    specs.push(vaatwasser.plaatsing === "inbouw" ? "Inbouw" : "Vrijstaand");
  }

  if (vaatwasser.energieLabel) {
    specs.push(`Energielabel ${vaatwasser.energieLabel}`);
  }

  if (vaatwasser.geluidDb) {
    specs.push(`${vaatwasser.geluidDb} dB`);
  }
...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "De vaatwasser die het beste bij je past!"
      : "De vaatwassers die het beste bij je passen!";
  }
}

function displayOtherMatchesRedesign(filteredMatchedVaatwassers) {
  resetProvidersRegistry();
  const container = qs("#otherMatchesGrid");
  if (!container) return;

  if (filteredMatchedVaatwassers.length === 0) {
    container.innerHTML = '<p class="no-matches">Geen passende vaatwassers gevonden.</p>';
    return;
  }

  const prices = filteredMatchedVaatwassers.map(v => parsePrice(v.prijs));
  const minPrice = Math.min(...prices);

  container.innerHTML = filteredMatchedVaatwassers
    .map((vaatwasser, index) => {
      const price = parsePrice(vaatwasser.prijs);
      const isCheapest = price === minPrice;
      const specs = buildSpecList(vaatwasser);
      const specsText = specs.join(" • ");
      const points = buildResultPoints(vaatwasser, currentAnswers);
      ...
      `).join("");
      const providersHtml = buildProvidersHtml(vaatwasser.aanbieders);

      return `
        <article class="tv-card${isCheapest ? " is-cheapest" : ""}" data-match-index="${index}">
          <div class="tv-card-image" aria-hidden="true">
            <img src="${vaatwasser.afbeelding || ''}" alt="" role="presentation" ${index < 4 ? 'fetchpriority="high" loading="eager"' : 'loading="lazy"'} onerror="this.onerror=null;this.src=window.IMG_FALLBACK;">
            <button class="tv-preview-btn" type="button" aria-label="Afbeelding vergroten" data-preview-src="${vaatwasser.afbeelding || ''}" data-preview-name="${vaatwasser.naam}" data-preview-imgs="${JSON.stringify(vaatwasser.afbeeldingen || []).replace(/"/g, '&quot;')}">
              <i data-lucide="eye"></i>
            </button>
          </div>
          <div class="tv-card-body">
            ${isCheapest ? '<span class="tv-card-cheapest-badge">Goedkoopste keuze</span>' : ''}
            <h3 class="tv-card-name">${vaatwasser.naam}</h3>
...
export function initResultPage() {
  ...
  const bestMatchData = localStorage.getItem("vaatwasser_bestMatch");
  const filteredData  = localStorage.getItem("vaatwasser_filteredMatchedVaatwassers");
  const answersData   = localStorage.getItem("vaatwasser_answers");

  if (!bestMatchData) return;

  const rawFilteredMatchedVaatwassers = filteredData ? JSON.parse(filteredData) : [];
  const answers                       = answersData  ? JSON.parse(answersData)  : null;

  const { result: filteredMatchedVaatwassers } = applyMinAanbiedersCascade(
    Array.isArray(rawFilteredMatchedVaatwassers) ? rawFilteredMatchedVaatwassers : []
  );

  currentAnswers = answers;
  baseMatches    = filteredMatchedVaatwassers;
