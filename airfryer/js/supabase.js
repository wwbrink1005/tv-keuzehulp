...
/**
 * Extracts the first (decimal) number from strings like "9 l" → 9, "1,5 kg" → 1.5.
 */
...
/**
 * "1,5 kg" → 1500, "800 g" → 800. capaciteit_gram staat als vrije Icecat-
 * tekst met wisselende eenheid, dus altijd naar gram normaliseren.
 */
function parseCapaciteitGram(value) {
  const num = parseFirstFloat(value);
  if (num === null) return null;
  return /kg/i.test(String(value)) ? num * 1000 : num;
}
...
/**
 * Adapts a single airfryers row to the shape that normalizeProducts() expects.
 */
function adaptRow(row) {
  return {
    ...
    titel:               row.titel ?? "",
    merk:                row.merk,
    capaciteitLiter:      parseFirstFloat(row.capaciteit_liter),
    capaciteitGram:       parseCapaciteitGram(row.capaciteit_gram),
    aantalPersonen:       parseFirstFloat(row.aantal_personen),
    constructietype:      row.constructietype ?? "",
    vermogenWatt:         parseFirstFloat(row.vermogen_watt),
    aantalProgrammas:     parseFirstFloat(row.aantal_programmas),
    temperatuurMin:       parseFirstFloat(row.temperatuur_min),
    temperatuurMax:       parseFirstFloat(row.temperatuur_max),
    frituurfunctie:       parseJaNee(row.frituurfunctie),
    bakfunctie:           parseJaNee(row.bakfunctie),
    grillen:              parseJaNee(row.grillen),
    braadfunctie:         parseJaNee(row.braadfunctie),
    stoomfunctie:         parseJaNee(row.stoomfunctie),
    dehydratiefunctie:    parseJaNee(row.dehydratiefunctie),
    warmhoudfunctie:      parseJaNee(row.warmhoudfunctie),
    kijkglas:             parseJaNee(row.kijkglas),
    display:              parseJaNee(row.display),
    displayType:          row.display_type ?? "",
    bediening:            row.bediening ?? "",
    digitaleTimer:        parseJaNee(row.digitale_timer),
    vaatwasserbestendig:  parseJaNee(row.vaatwasserbestendig),
    kleur:                row.kleur ?? "",
    breedte:              parseFirstFloat(row.breedte),
    diepte:               parseFirstFloat(row.diepte),
    hoogte:               parseFirstFloat(row.hoogte),
    gewicht:              parseFirstFloat(row.gewicht),
...
/**
 * Fetches all rows from the `airfryers` table with offset-based pagination.
 */
async function fetchAll() {
  ...
  while (true) {
    const url = `${SUPABASE_URL}/rest/v1/airfryers?order=ean&limit=${PAGE_SIZE}&offset=${offset}`;
    const response = await fetch(url, { headers: HEADERS });

    if (!response.ok) {
      throw new Error(
        `Supabase fetch mislukt (airfryers): ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Fetches all airfryers and returns them in the shape normalizeProducts() expects.
 */
