...
function buildSpecList(airfryer) {
  const specs = [];

  if (airfryer.capaciteitLiter) {
    specs.push(`${airfryer.capaciteitLiter} liter`);
  }

  if (airfryer.constructietype) {
    specs.push(airfryer.constructietype === "Dubbel" ? "Dubbele mand" : "Enkele mand");
  }

  if (airfryer.vermogenWatt) {
    specs.push(`${airfryer.vermogenWatt} W`);
  }

  if (airfryer.aantalProgrammas) {
    specs.push(`${airfryer.aantalProgrammas} programma's`);
  }
...
function displayOtherMatchesRedesign(filteredMatchedAirfryers) {
  resetProvidersRegistry();
  const container = qs("#otherMatchesGrid");
  if (!container) return;

  if (filteredMatchedAirfryers.length === 0) {
    container.innerHTML = '<p class="no-matches">Geen passende airfryers gevonden.</p>';
    return;
  }

  const prices = filteredMatchedAirfryers.map(a => parsePrice(a.prijs));
  const minPrice = Math.min(...prices);

  container.innerHTML = filteredMatchedAirfryers
    .map((airfryer, index) => {
...
export function initResultPage() {
  ...
  const bestMatchData = localStorage.getItem("airfryer_bestMatch");
  const filteredData  = localStorage.getItem("airfryer_filteredMatchedAirfryers");
