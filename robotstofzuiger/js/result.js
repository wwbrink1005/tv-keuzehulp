...
function buildSpecList(robot) {
  const specs = [];

  if (robot.natDweilen === "Ja") {
    specs.push("Zuigen + dweilen");
  }

  if (robot.zuigkracht) {
    specs.push(`${robot.zuigkracht} Pa`);
  }

  if (robot.looptijdMin) {
    specs.push(`${robot.looptijdMin} min looptijd`);
  }

  if (robot.geluidDb) {
    specs.push(`${robot.geluidDb} dB`);
  }
...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "De robotstofzuiger die het beste bij je past!"
      : "De robotstofzuigers die het beste bij je passen!";
  }
}

function displayOtherMatchesRedesign(filteredMatchedRobotstofzuigers) {
  resetProvidersRegistry();
  const container = qs("#otherMatchesGrid");
  if (!container) return;

  if (filteredMatchedRobotstofzuigers.length === 0) {
    container.innerHTML = '<p class="no-matches">Geen passende robotstofzuigers gevonden.</p>';
    return;
  }

  const prices = filteredMatchedRobotstofzuigers.map(r => parsePrice(r.prijs));
  const minPrice = Math.min(...prices);

  container.innerHTML = filteredMatchedRobotstofzuigers
    .map((robot, index) => {
      const price = parsePrice(robot.prijs);
      const isCheapest = price === minPrice;
      const specs = buildSpecList(robot);
      const specsText = specs.join(" • ");
      const points = buildResultPoints(robot, currentAnswers);
      ...
      `).join("");
      const providersHtml = buildProvidersHtml(robot.aanbieders);

      return `
        <article class="tv-card${isCheapest ? " is-cheapest" : ""}" data-match-index="${index}">
          <div class="tv-card-image" aria-hidden="true">
            <img src="${robot.afbeelding || ''}" alt="" role="presentation" ${index < 4 ? 'fetchpriority="high" loading="eager"' : 'loading="lazy"'} onerror="this.onerror=null;this.src=window.IMG_FALLBACK;">
            <button class="tv-preview-btn" type="button" aria-label="Afbeelding vergroten" data-preview-src="${robot.afbeelding || ''}" data-preview-name="${robot.naam}" data-preview-imgs="${JSON.stringify(robot.afbeeldingen || []).replace(/"/g, '&quot;')}">
              <i data-lucide="eye"></i>
            </button>
          </div>
          <div class="tv-card-body">
            ${isCheapest ? '<span class="tv-card-cheapest-badge">Goedkoopste keuze</span>' : ''}
            <h3 class="tv-card-name">${robot.naam}</h3>
...
export function initResultPage() {
  ...
  const bestMatchData = localStorage.getItem("robotstofzuiger_bestMatch");
  const filteredData  = localStorage.getItem("robotstofzuiger_filteredMatchedRobotstofzuigers");
