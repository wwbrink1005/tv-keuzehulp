...
/**
 * Extracts the first integer from strings like "180 min" → 180, "65 dB" → 65.
 */
...
/**
 * Extracts the first (decimal) number from strings like "9000 Pa" → 9000.
 */
...
/**
 * Adapts a single robotstofzuigers row to the shape that normalizeProducts() expects.
 */
function adaptRow(row) {
  return {
    ...
    titel:                   row.titel ?? "",
    merk:                    row.merk,
    navigatieType:           row.navigatie_type ?? "",
    natDweilen:              parseJaNee(row.nat_dweilen),
    zuigkracht:              parseFirstInt(row.zuigkracht),
    looptijdMin:             parseFirstInt(row.looptijd_min),
    geluidDb:                parseFirstFloat(row.geluid_db),
    zelflegend:              parseJaNee(row.zelflegend),
    obstakeldetectie:        parseJaNee(row.obstakeldetectie),
    hepaFilter:              parseJaNee(row.hepa_filter),
    wifi:                    parseJaNee(row.wifi),
    alexa:                   parseJaNee(row.alexa),
    googleAssistent:         parseJaNee(row.google_assistent),
    stofcapaciteit:          row.stofcapaciteit ?? "",
    accuCapaciteit:          row.accu_capaciteit ?? "",
    hoogteMm:                parseFirstFloat(row.hoogte),
    basisstationAfmetingen:  row.basisstation_afmetingen ?? "",
    diameterCm:              parseFirstFloat(row.diameter),
    kleur:                   row.kleur ?? "",
...
/**
 * Fetches all rows from the `robotstofzuigers` table with offset-based pagination.
 */
async function fetchAll() {
  ...
  while (true) {
    const url = `${SUPABASE_URL}/rest/v1/robotstofzuigers?order=ean&limit=${PAGE_SIZE}&offset=${offset}`;
    const response = await fetch(url, { headers: HEADERS });

    if (!response.ok) {
      throw new Error(
        `Supabase fetch mislukt (robotstofzuigers): ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Fetches all robotstofzuigers and returns them in the shape normalizeProducts() expects.
 */
