import { matchRobotstofzuigers } from "./matching.js";
...
function buildAnswers() {
  return {
    navigatie:        qs('input[name="navigatie"]:checked')?.value ?? "",
    dweilen:          qs('input[name="dweilen"]:checked')?.value ?? "",
    woninggrootte:    qs('input[name="woninggrootte"]:checked')?.value ?? "",
...
function handleStartMatching() {
  ...
  prefetchProducts()
    .then(rawProducts => {
      const robots = normalizeProducts(rawProducts ?? []);
      const result = matchRobotstofzuigers(robots, answers);
...
export function initQuizPage() {
  ...
  qs("#to-question-2")?.addEventListener("click", () => {
    const checked = qs('input[name="navigatie"]:checked');
  ...
  qs("#to-question-3")?.addEventListener("click", () => {
    const checked = qs('input[name="dweilen"]:checked');
    if (!checked) return alert("Kies een antwoord");
    showQuestion(3);
  });
  ...
  qs("#to-question-4")?.addEventListener("click", () => {
    const checked = qs('input[name="woninggrootte"]:checked');
    if (!checked) return alert("Kies een antwoord");
    showQuestion(4);
  });
