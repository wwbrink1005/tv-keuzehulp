...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "De monitor die het beste bij je past!"
      : "De monitoren die het beste bij je passen!";
  }
}
...
function displayOtherMatchesRedesign(filteredMatchedMonitors) {
  ...
  container.innerHTML = filteredMatchedMonitors
    .map((monitor, index) => {
      ...
      `).join("");
      const providersHtml = buildProvidersHtml(monitor.aanbieders);

      return `
        <article class="tv-card${isCheapest ? " is-cheapest" : ""}" data-match-index="${index}">
          <div class="tv-card-image" aria-hidden="true">
            <img src="${monitor.afbeelding || ''}" alt="" role="presentation" ${index < 4 ? 'fetchpriority="high" loading="eager"' : 'loading="lazy"'} onerror="this.onerror=null;this.src=window.IMG_FALLBACK;">
...
export function initResultPage() {
  ...
  const rawFilteredMatchedMonitors = filteredData ? JSON.parse(filteredData) : [];
  const answers                    = answersData  ? JSON.parse(answersData)  : null;

  const { result: filteredMatchedMonitors } = applyMinAanbiedersCascade(
    Array.isArray(rawFilteredMatchedMonitors) ? rawFilteredMatchedMonitors : []
  );

  currentAnswers = answers;
  baseMatches    = filteredMatchedMonitors;
