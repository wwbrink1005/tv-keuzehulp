...
/**
 * Extracts the first integer from strings like "220 min" → 220, "64 dB" → 64.
 */
...
/**
 * Adapts a single wasdrogers row to the shape that normalizeProducts() expects.
 */
function adaptRow(row) {
  return {
    ...
    antikreuk:           parseJaNee(row.antikreuk),
    vochtsensor:         parseJaNee(row.vochtsensor),
    droogprogrammas:     row.droogprogrammas ?? "",
...
/**
 * Fetches all rows from the `wasdrogers` table with offset-based pagination.
 */
async function fetchAll() {
  ...
  while (true) {
    const url = `${SUPABASE_URL}/rest/v1/wasdrogers?order=ean&limit=${PAGE_SIZE}&offset=${offset}`;
    const response = await fetch(url, { headers: HEADERS });

    if (!response.ok) {
      throw new Error(
        `Supabase fetch mislukt (wasdrogers): ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Fetches all wasdrogers and returns them in the shape normalizeProducts() expects.
 */
