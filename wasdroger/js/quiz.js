...
// dezelfde ranges die de matching-logica gebruikt (data.js).
...
function handleStartMatching() {
  ...
  prefetchProducts()
    .then(rawProducts => {
      const wasdrogers = normalizeProducts(rawProducts ?? []);
      const result = matchWasdrogers(
        wasdrogers,
      ...
      localStorage.setItem("wasdroger_selectedCapaciteitGroup",   quizState.selectedCapaciteitGroup ?? "");
...
export function initQuizPage() {
  ...
  qsa('input[name="capaciteitGroup"]').forEach(radio => {
    radio.addEventListener("change", updateWasdrogerDisplay);
  });
