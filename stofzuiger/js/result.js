...
function buildSpecList(stofzuiger) {
  const specs = [];

  specs.push(stofzuiger.stofzuigerType === "Steelstofzuiger" ? "Steelstofzuiger" : "Cilinderstofzuiger");

  if (stofzuiger.containerType) {
    specs.push(stofzuiger.containerType === "Stofzak" ? "Met stofzak" : "Zonder stofzak");
  }

  if (stofzuiger.merk) {
    specs.push(stofzuiger.merk);
  }
...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "De stofzuiger die het beste bij je past!"
      : "De stofzuigers die het beste bij je passen!";
  }
}

function displayOtherMatchesRedesign(filteredMatchedStofzuigers) {
  resetProvidersRegistry();
  const container = qs("#otherMatchesGrid");
  if (!container) return;

  if (filteredMatchedStofzuigers.length === 0) {
    container.innerHTML = '<p class="no-matches">Geen passende stofzuigers gevonden.</p>';
    return;
  }

  const prices = filteredMatchedStofzuigers.map(s => parsePrice(s.prijs));
  const minPrice = Math.min(...prices);

  container.innerHTML = filteredMatchedStofzuigers
    .map((stofzuiger, index) => {
      const price = parsePrice(stofzuiger.prijs);
      const isCheapest = price === minPrice;
      const specs = buildSpecList(stofzuiger);
      const specsText = specs.join(" • ");
      const points = buildResultPoints(stofzuiger, currentAnswers);
      ...
      `).join("");
      const providersHtml = buildProvidersHtml(stofzuiger.aanbieders);

      return `
        <article class="tv-card${isCheapest ? " is-cheapest" : ""}" data-match-index="${index}">
          <div class="tv-card-image" aria-hidden="true">
            <img src="${stofzuiger.afbeelding || ''}" alt="" role="presentation" ${index < 4 ? 'fetchpriority="high" loading="eager"' : 'loading="lazy"'} onerror="this.onerror=null;this.src=window.IMG_FALLBACK;">
            <button class="tv-preview-btn" type="button" aria-label="Afbeelding vergroten" data-preview-src="${stofzuiger.afbeelding || ''}" data-preview-name="${stofzuiger.naam}" data-preview-imgs="${JSON.stringify(stofzuiger.afbeeldingen || []).replace(/"/g, '&quot;')}">
              <i data-lucide="eye"></i>
            </button>
          </div>
          <div class="tv-card-body">
            ${isCheapest ? '<span class="tv-card-cheapest-badge">Goedkoopste keuze</span>' : ''}
            <h3 class="tv-card-name">${stofzuiger.naam}</h3>
...
export function initResultPage() {
  ...
  const bestMatchData = localStorage.getItem("stofzuiger_bestMatch");
  const filteredData  = localStorage.getItem("stofzuiger_filteredMatchedStofzuigers");
  const answersData   = localStorage.getItem("stofzuiger_answers");

  if (!bestMatchData) return;

  const rawFilteredMatchedStofzuigers = filteredData ? JSON.parse(filteredData) : [];
  const answers                       = answersData  ? JSON.parse(answersData)  : null;

  currentAnswers = answers;
  baseMatches    = Array.isArray(rawFilteredMatchedStofzuigers) ? rawFilteredMatchedStofzuigers : [];
