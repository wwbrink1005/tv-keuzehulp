...
function buildAnswers() {
  return {
    ...
    licht:          qs('input[name="licht"]:checked')?.value ?? "",
    beeldkwaliteit: qs('input[name="beeldkwaliteit"]:checked')?.value ?? "",
    draagbaar:      qs('input[name="draagbaar"]:checked')?.value ?? "",
...
function handleStartMatching() {
  ...
  prefetchProducts()
    .then(rawProducts => {
      const beamers = normalizeProducts(rawProducts ?? []);
      const result = matchBeamers(beamers, answers);
...
export function initQuizPage() {
  ...
  qs("#to-question-3")?.addEventListener("click", () => {
    const checked = qs('input[name="licht"]:checked');
    if (!checked) return alert("Kies een antwoord");
    showQuestion(3);
  });
  ...
  qs("#to-question-4")?.addEventListener("click", () => {
    const checked = qs('input[name="beeldkwaliteit"]:checked');
    if (!checked) return alert("Kies een antwoord");
    showQuestion(4);
  });
  ...
  qs("#to-question-5")?.addEventListener("click", () => {
    const checked = qs('input[name="draagbaar"]:checked');
    if (!checked) return alert("Kies een antwoord");
    showQuestion(5);
  });
