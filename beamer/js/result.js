...
function buildSpecList(beamer) {
  const specs = [];

  if (beamer.helderheidLumen) {
    specs.push(`${beamer.helderheidLumen} lumen`);
  }

  if (beamer.resolutie) {
    specs.push(beamer.resolutie);
  }

  if (beamer.gewichtKg !== null) {
    specs.push(`${beamer.gewichtKg} kg`);
  }
...
function updateMatchCount(count) {
  ...
  if (titleEl) {
    titleEl.textContent = count === 1
      ? "De beamer die het beste bij je past!"
      : "De beamers die het beste bij je passen!";
  }
}

function displayOtherMatchesRedesign(filteredMatchedBeamers) {
  resetProvidersRegistry();
  const container = qs("#otherMatchesGrid");
  if (!container) return;

  if (filteredMatchedBeamers.length === 0) {
    container.innerHTML = '<p class="no-matches">Geen passende beamers gevonden.</p>';
    return;
  }

  const prices = filteredMatchedBeamers.map(b => parsePrice(b.prijs));
  const minPrice = Math.min(...prices);

  container.innerHTML = filteredMatchedBeamers
    .map((beamer, index) => {
      const price = parsePrice(beamer.prijs);
      const isCheapest = price === minPrice;
      const specs = buildSpecList(beamer);
      const specsText = specs.join(" • ");
      const points = buildResultPoints(beamer, currentAnswers);
      ...
      `).join("");
      const providersHtml = buildProvidersHtml(beamer.aanbieders);

      return `
        <article class="tv-card${isCheapest ? " is-cheapest" : ""}" data-match-index="${index}">
          <div class="tv-card-image" aria-hidden="true">
            <img src="${beamer.afbeelding || ''}" alt="" role="presentation" ${index < 4 ? 'fetchpriority="high" loading="eager"' : 'loading="lazy"'} onerror="this.onerror=null;this.src=window.IMG_FALLBACK;">
            <button class="tv-preview-btn" type="button" aria-label="Afbeelding vergroten" data-preview-src="${beamer.afbeelding || ''}" data-preview-name="${beamer.naam}" data-preview-imgs="${JSON.stringify(beamer.afbeeldingen || []).replace(/"/g, '&quot;')}">
              <i data-lucide="eye"></i>
            </button>
          </div>
          <div class="tv-card-body">
            ${isCheapest ? '<span class="tv-card-cheapest-badge">Goedkoopste keuze</span>' : ''}
            <h3 class="tv-card-name">${beamer.naam}</h3>
...
export function initResultPage() {
  ...
  const bestMatchData = localStorage.getItem("beamer_bestMatch");
  const filteredData  = localStorage.getItem("beamer_filteredMatchedBeamers");
  const answersData   = localStorage.getItem("beamer_answers");

  if (!bestMatchData) return;

  const rawFilteredMatchedBeamers = filteredData ? JSON.parse(filteredData) : [];
  const answers                   = answersData  ? JSON.parse(answersData)  : null;

  const { result: filteredMatchedBeamers } = applyMinAanbiedersCascade(
    Array.isArray(rawFilteredMatchedBeamers) ? rawFilteredMatchedBeamers : []
  );

  currentAnswers = answers;
  baseMatches    = filteredMatchedBeamers;
